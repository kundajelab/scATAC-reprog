...
NUM_CLUSTERS=$3
CLUSTER_TAGALIGNS_PREFIX=$4 # prefix to the clusters e.g. (full path) "...snapATAC_sessions/20200125_n76770/cluster_tagAligns/cluster_idx"
OUTFILE=$5
...
getcounts () {
 ...
 # $2 is CLUSTER_TAGALIGNS_PREFIX passed through xargs
 cur_tagalign=$2$1.tagAlign.gz
 echo $cur_tagalign

 bedtools coverage -counts -a peaks.sorted.bed \
        -b <(zcat ${cur_tagalign} | cut -f1-3 | sort -k1,1 -k2,2n)  -sorted \
...
seq 1 $numfiles | xargs -I{} -P $THREADS bash -c 'getcounts "$@"' _ {} ${CLUSTER_TAGALIGNS_PREFIX}
