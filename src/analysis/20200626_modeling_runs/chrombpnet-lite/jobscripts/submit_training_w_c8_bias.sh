...
for i in 1 11 8
 do
  mkdir -p $BASE/models/$RUNNAME/chrombpnet_c8_bias/cluster_idx$i
 done
...
for i in 1 11 8
 do
  sbatch --job-name train_c$i \
         --output $BASE/models/$RUNNAME/chrombpnet_c8_bias/cluster_idx$i/log.txt \
         --error $BASE/models/$RUNNAME/chrombpnet_c8_bias/cluster_idx$i/err.txt \
         --mem 200G \
         ...
         --output-dir $BASE/models/$RUNNAME/chrombpnet_c8_bias/cluster_idx$i/ \
         --test-chr "chr1" --val-chr "chr8" "chr10" \
         --bias-model $BASE/models/$RUNNAME/bias/cluster_idx8/a9WiZ.h5 \
         --max-jitter 500 
 done
